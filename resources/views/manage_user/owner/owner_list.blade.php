<x-layout>
    ...
    <div class="card p-4">
        <div class="card-header">
            <a href="{{route('owner.view')}}" class="dt-button create-new btn btn-primary"
                tabindex="0" aria-controls="DataTables_Table_0" style="float: right;">


                <span><i class="bx bx-plus me-sm-2"></i>
                    <span class="d-none d-sm-inline-block">Add Owner</span>
                </span></a>
            <h5 style="width: 50%;">
                ...
                <tbody>
                    @foreach ($owner as $list)
                        <tr>
                            ...
                            <td>{{ $list->location }}</td>
                           <td>
                            <select name="" id="" onchange="change_status_owner($(this).find('option:selected').attr('data-id'),$(this).val())"  class="form-control">
                            ...
                            <td>
                                <a class="btn ntn-sm btn-primary" href="{{ Route('owner.edit', $list->id) }}" title="Edit"><i class='bx bx-edit'></i></a>
...
function change_status_owner(owner_id,status)
        {
            $.ajax({
                    type: "post",
                    dataType: "json",
                    url: "{{url('admin/owner-change-status')}}",
                    data: { status, owner_id,'_token':'{{csrf_token()}}'},
        ...
        function deleted(id)
        {
            ...
            .then((willDelete) => {
             if (willDelete) {
                window.location="{{url('/admin/owner-delete') }}/"+id;
             }
            });
        }
      </script>
