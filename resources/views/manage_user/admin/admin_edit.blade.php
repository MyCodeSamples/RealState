<x-layout>
    ...
    <form action="{{Route('admin.update', $admin->id)}}" method="POST" enctype="multipart/form-data" class="pl-5" >
        <h4> Edit Admin!</h4>
        <hr/>
        {{env('app.user_type')}}
        @csrf
    ...
    <select name="provider" id="provider" class="form-control">
        ...
        <option value="Builder Developers" {{ ($admin->provider=='Builder Developers')?'selected':'' }}>Builder Developers</option>
    ...
    <div class="row form-group">
        ...
        {{-- <div class="col-md-6">
            ...
            <input type="password" value="{{$admin->password}}" name="password" class="form-control" placeholder="Enter Password*">
            @error('password')
            <Span class="error-field text-danger">{{$message}}</Span>
        @enderror
        </div> --}}
    ...
    <div class="row form-group">
        ...
        <div class="col-md-6">
            ...
            <input type="number" value="{{$admin->phone}}" name="phone" class="form-control" placeholder="Enter Phone*">
            @error('phone')
            <Span class="error-field text-danger">{{$message}}</Span>
        @enderror
        </div>
        ...
        <div class="col-md-6">
            ...
            <input type="text" value="{{$admin->website_link}}" name="website_link" class="form-control" placeholder="Enter Website Link">
            @error('website_link')
            <Span class="error-field text-danger">{{$message}}</Span>
        @enderror
        </div>
    ...
    <div class="row form-group">
        <div class="col-md-6">
            ...
            <textarea type="text" class="form-control" name="description" placeholder="Enter Description...">{{$admin->description}}</textarea>
            @error('description')
            <Span class="error-field text-danger">{{$message}}</Span>
        @enderror
        </div>
            <div class="col-md-6">
                ...
                <input type="text" name="location" value="{{$admin->location}}" class="form-control" placeholder="Enter Location">
                @error('location')
            <Span class="error-field text-danger">{{$message}}</Span>
            @enderror
            </div>
            ...
            <div class="col-md-6">
                ...
                <input type="hidden" name="oldlogo" id="oldlogo" class="form-control" value="{{$admin->logo}}">
                <input type="file" name="logo" id="logo" class="form-control">
                @error('logo')
                    <span class="field_error text-danger">{{ $message }}</span>
                @enderror
                @if(!empty($admin->logo))
                <img src="{{URL::to($admin->logo)}}">
                @endif
            </div>
            <div class="col-md-4">
