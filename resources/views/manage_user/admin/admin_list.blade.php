<x-layout>
    ...
    <div class="card p-4">
        ...
        <div class="card-header">
            <a href="{{route('admin.view')}}" class="dt-button create-new btn btn-primary"
                ...
                <tbody>
                    @foreach ($admin as $list)
                        <tr>
                            ...
                            <td>{{ $list->location }}</td>
                            <td>
                                <select name="" id="" onchange="change_status_admin($(this).find('option:selected').attr('data-id'),$(this).val())" style="size: 40px;">
                           ...
                           <td>
                            ...
                            <td>
                                <a class="btn ntn-sm btn-primary" href="{{ Route('admin.edit', $list->id) }}" title="Edit"><i class='bx bx-edit'></i></a>
    ...
    <script type="text/javascript">
        function change_status_admin(admin_id,status)
        {
            $.ajax({
                    type: "post",
                    dataType: "json",
                    url: "{{url('admin/admin-change-status')}}",
                    data: { status, admin_id,'_token':'{{csrf_token()}}'},
        ...
        function deleted(id)
        {
            ...
            .then((willDelete) => {
             if (willDelete) {
                window.location="{{url('/admin/admin-delete') }}/"+id;
             }
            });
        }
      </script>
