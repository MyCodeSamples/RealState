<x-layout>
    ...
    <form action="{{Route('agent.update', $agent->id)}}" method="POST" enctype="multipart/form-data" class="pl-5" >
        <h4> Edit Agent!</h4>
        <hr/>
        @csrf
    ...
    @enderror
     ...
     <input type="text" name="name" value="{{$agent->name}}" class="form-control" placeholder="Enter Name">
     @error('name')
        <Span class="error-field text-danger">{{$message}}</Span>
    @enderror
        </div>
        <div class="col-md-6">
            ...
            <input type="email" name="email" value="{{$agent->email}}" class="form-control" placeholder="Enter Email">
            @error('email')
            <Span class="error-field text-danger">{{$message}}</Span>
        @enderror
        </div>
    ...
    <div class="row form-group">
        ...
        {{-- <div class="col-md-6">
            ...
            <input type="password" value="{{$agent->password}}" name="password" class="form-control" placeholder="Enter Password*">
            @error('password')
            <Span class="error-field text-danger">{{$message}}</Span>
        @enderror
        </div> --}}
    ...
    <div class="row form-group">
        ...
        <div class="col-md-6">
            ...
            <input type="number" value="{{$agent->phone}}" name="phone" class="form-control" placeholder="Enter Phone*">
            @error('phone')
            <Span class="error-field text-danger">{{$message}}</Span>
        @enderror
        </div>
        ...
        <div class="col-md-6">
            ...
            <input type="text" value="{{$agent->website_link}}" name="website_link" class="form-control" placeholder="Enter Website Link">
            @error('website_link')
            <Span class="error-field text-danger">{{$message}}</Span>
        @enderror
        </div>
    ...
    <div class="row form-group">
        <div class="col-md-6">
            ...
            <textarea type="text" class="form-control" name="description" placeholder="Enter Description...">{{$agent->description}}</textarea>
            @error('description')
            <Span class="error-field text-danger">{{$message}}</Span>
        @enderror
        </div>
            <div class="col-md-6">
                ...
                <input type="text" name="location" value="{{$agent->location}}" class="form-control" placeholder="Enter Location">
                @error('location')
            <Span class="error-field text-danger">{{$message}}</Span>
            @enderror
            </div>
            ...
            <div class="col-md-6">
                ...
                <input type="hidden" name="oldlogo" id="oldlogo" class="form-control" value="{{$agent->logo}}">
                <input type="file" name="logo" id="logo" class="form-control">
                @error('logo')
                    <span class="field_error text-danger">{{ $message }}</span>
                @enderror
                @if(!empty($agent->logo))
                <img src="{{URL::to($agent->logo)}}">
                @endif
            </div>
            <div class="col-md-4">
