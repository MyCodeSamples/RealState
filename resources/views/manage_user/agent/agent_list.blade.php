<x-layout>
    ...
    <div class="card p-4">
        <div class="card-header">
            <a href="{{route('agent.view')}}" class="dt-button create-new btn btn-primary"
                ...
                <tbody>
                    @foreach ($agent as $list)
                        <tr>
                            ...
                            <td>{{ $list->location }}</td>
                            <td>
                                <select class="form-control" name="" id="" onchange="change_status_agent($(this).find('option:selected').attr('data-id'),$(this).val())" style="size: 40px;">
                            ...
                            <td>
                                <a class="btn ntn-sm btn-primary" href="{{ Route('agent.edit', $list->id) }}" title="Edit"><i class='bx bx-edit'></i></a>
...
function change_status_agent(agent_id,status)
        {
            $.ajax({
                    type: "post",
                    dataType: "json",
                    url: "{{url('admin/agent-change-status')}}",
        ...
        function deleted(id)
        {
            ...
            .then((willDelete) => {
             if (willDelete) {
                window.location="{{url('/admin/agent-delete') }}/"+id;
             }
            });
        }
      </script>
