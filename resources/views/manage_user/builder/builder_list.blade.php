<x-layout>
    ...
    <div class="card p-4">
        <div class="card-header">
            <a href="{{route('builder.view')}}" class="dt-button create-new btn btn-primary"
                tabindex="0" aria-controls="DataTables_Table_0" style="float: right;">


                <span><i class="bx bx-plus me-sm-2"></i>
                    <span class="d-none d-sm-inline-block">Add Builder</span>
                </span></a>
            <h5 style="width: 50%;">
                ...
                <tbody>
                    @foreach ($builder as $list)
                        <tr>
                            ...
                            <td>
                                <select class="form-control" name="" id="" onchange="change_status_builder($(this).find('option:selected').attr('data-id'),$(this).val())" style="size: 40px;">
                            ...
                            <td>
                                <a class="btn ntn-sm btn-primary" href="{{ Route('builder.edit', $list->id) }}" title="Edit"><i class='bx bx-edit'></i></a>
    ...
    <script type="text/javascript">
     function change_status_builder(builder_id,status)
        {
            $.ajax({
                    type: "post",
                    dataType: "json",
                    url: "{{url('admin/builder-change-status')}}",
                    data: { status, builder_id,'_token':'{{csrf_token()}}'},
        ...
        function deleted(id)
        {
            ...
            .then((willDelete) => {
             if (willDelete) {
                window.location="{{url('/admin/builder-delete') }}/"+id;
             }
            });
        }
      </script>
